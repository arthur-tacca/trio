...
def test_notify_closing_from_host_after_events() -> None:
    # Regression test for the notify_closing variant of
    # https://github.com/python-trio/trio/issues/3500 (see also #3503): in the
    # same window as above, the kqueue backend's notify_closing tried to delete
    # a one-shot event that had already been consumed, and raised an OSError
    # into the host loop.
    a, b = socket.socketpair()
    with a, b:

        async def trio_main(in_host: InHost) -> None:
            def notify_closing_from_host() -> None:
                trio.lowlevel.notify_closing(a)

            in_host(notify_closing_from_host)
